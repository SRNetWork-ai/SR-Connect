...
```
ghcr.io/srnetwork-ai/sr-connect-web:latest
ghcr.io/srnetwork-ai/sr-connect-gateway:latest
```

چون مقادیر پیکربندی در **زمان اجرا** از `/api/config` خوانده می‌شوند (نه هنگام بیلد)،
یک ایمیج یکسان روی هر سروری با کلید و دامنهٔ خودش کار می‌کند.
